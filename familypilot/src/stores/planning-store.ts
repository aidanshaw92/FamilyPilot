...
}),{name:'familypilot-planning-v1',skipHydration:Platform.OS==='web'&&typeof window==='undefined',storage:createJSONStorage(()=>AsyncStorage),partialize:({families,options,saved})=>({families,options,saved}),onRehydrateStorage:()=>()=>usePlanningStore.setState({hydrated:true})}));
