...
export function FamilyEditor({initial,onSave,onCancel}:{initial:PlanningFamily;onSave:(f:PlanningFamily)=>void;onCancel:()=>void}) {
 ...
 return <Card style={formStyles.panel}>
   <Text variant="heading2">{initial.id==='mine'?'Your family':'Family planning details'}</Text>
   <Text variant="bodySmall" color={colors.text.secondary}>Use a town or postcode, not a street address. Only add another family’s details with their agreement.</Text>
   ...
   <Text variant="bodySmall" color={colors.text.secondary}>Times apply on the day you plan. Adjust them for that day. These are planning preferences; always follow your baby’s cues.</Text>
   {family.routines.map(r=><Card key={r.id} elevated={false} style={styles.routineCard}>
     ...
     <Button label="Remove routine" variant="ghost" size="sm" onPress={()=>change({routines:family.routines.filter(x=>x.id!==r.id)})}/>
   </Card>)}
...
const styles=StyleSheet.create({
 routineCard:{gap:10,backgroundColor:colors.background,borderWidth:1,borderColor:colors.border},
});
